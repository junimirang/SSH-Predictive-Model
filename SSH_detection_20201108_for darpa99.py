...
def hybrid_detection(x_test, y_test, y_pred_rf, y_pred_dtree):
    ...
    return metrics.accuracy_score(y_test, y_pred_hybrid), y_pred_hybrid
...
def loading_dataset(): ## After loading csv file, Pandas Data Frameset Generation ##
    # time.taken   c.ip   response.code  response.type  sc.byte    cs.byte    method URI    cs.host    cs.uri.port
    # cs_user_agent    sc_filter_result   category   destination.ip isp    region no_url
    ...
    return(X, K, L, Z, Y, X_compare, K_compare, L_compare, Z_compare, Y_compare, N_compare)


def PCA(X, K, L, Z, Y, K_compare, L_compare, Z_compare, Y_compare):
    features = X.T
    ...
    projected_X1 = 100*(projected_X1 - min(projected_X1))/(max(projected_X1)-min(projected_X1)) #PC value normalization
    #projected_X2 = 100*(projected_X2 - min(projected_X2))/(max(projected_X2)-min(projected_X2))
...
def SSH_prediction_model(result1, result2, i):
    ...
    ## Hybrid Modeling ##
    hybrid_accuracy1, label_hybrid1 = hybrid_detection(x_test, y_test, y_pred_rf100, y_pred_dtree4)
    ...
    hybrid_compare_accuracy1, label_hybrid1 = hybrid_detection(x_compare, y_compare, y_compare_predict_rf, y_compare_predict_dt)
    ...
    print(i,",",'Detection Rate of Compare DATA(RF):', ",", metrics.accuracy_score(y_compare, label_rf100))
    print(i,",",'Detection Rate of Compare DATA(DT):', ",", metrics.accuracy_score(y_compare, label_dt4))
    print(i, ",", 'Detection Rate of Compare DATA(Hybrid):', ",", metrics.accuracy_score(y_compare, label_hybrid1))
    ...
    label_dt5 = 0
    label_hybrid2 = 0
    return forest100, forest1000, dtree4, dtree5, hybrid_compare_accuracy1, hybrid_compare_accuracy2, y_compare, label_rf100, label_rf1000, label_dt4, label_dt5, label_hybrid1, label_hybrid2


def result_pred_output(y_compare, label_rf100, label_rf1000, label_dt4, label_dt5, label_hybrid1, label_hybrid2,N_compare, i):
    ...
    #y_pred_pivot = y_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_RF', 'Label_DT4', 'Label_DT5','Label_HYBRID'], aggfunc="count")
    ...
    #y_pred_pivot_rf1000 = y_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_RF100'], aggfunc="count")
    #y_pred_pivot_dt5 = y_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_DT5'], aggfunc="count")
    #y_pred_pivot_hybrid2 = y_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_HYBRID_1000_5'], aggfunc="count")
    ...
    print(i,',', "Total Detection Rate(RF):",',', r1[1])
    print(i,',', "Total Detection Rate(DT):", ',', d1[1])
    print(i,',', "Total Detection Rate(Hybrid):",',', h1[1])
    print(i,',', "SSH Detection Rate(RF):",',', r1[0])
    print(i,',', "SSH Detection Rate(DT):", ',', d1[0])
    print(i,',', "SSH Detection Rate(Hybrid):",',', h1[0])
    print(i,',', "True Positive Gap of SSH_detection Rate(Hybrid-RF):",',', h1[0]-r1[0]) ## Hybrid? Rndom Forest? SSH ??? ??
    print(i, ',', "False Rate of total detection(RF):", ',', r1[2])
    print(i, ',', "False Rate of total detection(DT):", ',', d1[2])
    print(i, ',', "False Rate of total detection(Hybrid):", ',', h1[2])
    print(i, ',', "False Positive Rate of total detection(RF):", ',', r1[3])
    print(i, ',', "False Positive Rate of total detection(DT):", ',', d1[3])
    print(i, ',', "False Positive Rate of total detection(Hybrid):", ',', h1[3])
...
def ssh_count(y_pred_pivot): ## SSH Count with IP:Port
    ...
    ssh_detection = true_positive_count/ssh_site          ## true positive detection rate
    false_positive_rate = false_positive_count/etc_site
    true_negative_rate = true_negative_count/etc_site

    return ssh_detection, total_detection, false_rate, false_positive_rate, true_negative_rate
...
if __name__ == '__main__':
    ...
    X, K, L, Z, Y, X_compare, K_compare, L_compare, Z_compare, Y_compare, N_compare = loading_dataset()
    result1, result2, result_PCA = PCA(X, K, L, Z, Y, K_compare, L_compare, Z_compare, Y_compare)
    sys.stdout = open('Result_output/output100.csv', 'w')       # Print as file #
    i=0
    performance_compare = 0
    ...
    while i<100:  ## Repeating N times for Predictive model approval
        ...
        forest100, forest1000, dtree4, dtree5, hybrid_compare_accuracy1, hybrid_compare_accuracy2, y_compare, label_rf100, label_rf1000, label_dt4, label_dt5, label_hybrid1, label_hybrid2 = SSH_prediction_model(result1, result2,i)
        r1, d1, h1 = result_pred_output(y_compare, label_rf100, label_rf1000, label_dt4, label_dt5, label_hybrid1, label_hybrid2, N_compare, i)
        ssh_gap = h1[0] - r1[0]
        RF_DT_gap = d1[0] - r1[0]  # RF - DT 간 편차가 새로운 탐지율에 미치는 영향
        false_rate = h1[2]
        false_positive_rate = h1[3]

        if hybrid_compare_accuracy1 >= performance_compare:   ## total row accuracy  SSH in Hybrid Model
            performance_compare = hybrid_compare_accuracy1
            case_max_row_rate = i
        ...
        if h1[0] >= performance_compare:  ## Largest IP:Port Detection rate in Hybrid Model
            performance_compare = h1[0]
