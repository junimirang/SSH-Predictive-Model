...
def loading_dataset(): ## After loading csv file, Pandas Data Frameset Generation ##
    ...
    df = pd.read_csv('base_20200109.csv', index_col=0)
    df_compare = pd.read_csv('compare2nd_labeled.csv', index_col=0)
    ...
    N = df[["destination.ip", "cs.uri.port", "no_url"]]
    ...
    N_compare = df_compare[["destination.ip", "cs.uri.port", "no_url"]]
...
def PCA(X, K, L, Z, Y, N, K_compare, L_compare, Z_compare, Y_compare, N_compare):
    ...
    result1['Label'] = Y
    result1[["destination.ip", "cs.uri.port"]] = N[["destination.ip", "cs.uri.port"]]
    ...
    result2[["destination.ip", "cs.uri.port"]] = N_compare[["destination.ip", "cs.uri.port"]]
...
def SSH_prediction_model(result1, result2, i):
    ...
    y = result1[['Label']]
    z = result1[['No_url']]
    n = result1[["destination.ip", "cs.uri.port"]]
    ...
    N_compare = result2[["destination.ip", "cs.uri.port"]]
...
def result_pred_output(y_compare, label_rf100, label_rf1000, label_dt4, label_dt5, label_hybrid1, label_hybrid2, N_compare, i, y_test, y_pred_rf100, y_pred_dtree4, y_pred_hybrid1, N, x_test, z_test, n_test):
    ...
    y_test_pred_pivot_rf100 = y_test_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_RF100'], aggfunc="count")
    y_test_pred_pivot_dt4 = y_test_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_DT4'], aggfunc="count")
    y_test_pred_pivot_hybrid1 = y_test_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_HYBRID_100_4'], aggfunc="count")
    #y_pred_pivot_rf1000 = y_pred_total.pivot_table('no_url', ['destination.ip.ip', 'cs.uri.port', 'Label'], ['Label_RF100'], aggfunc="count")
    ...
    #y_pred_pivot = y_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_RF', 'Label_DT4', 'Label_DT5','Label_HYBRID'], aggfunc="count")
    #y_pred_pivot.to_csv("pivot.csv", mode='w')
    y_pred_pivot_rf100 = y_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_RF100'], aggfunc="count")
    y_pred_pivot_dt4 = y_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_DT4'], aggfunc="count")
    y_pred_pivot_hybrid1 = y_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_HYBRID_100_4'], aggfunc="count")
    #y_pred_pivot_rf1000 = y_pred_total.pivot_table('no_url', ['destination.ip.ip', 'cs.uri.port', 'Label'], ['Label_RF100'], aggfunc="count")
    #y_pred_pivot_dt5 = y_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_DT5'], aggfunc="count")
    #y_pred_pivot_hybrid2 = y_pred_total.pivot_table('no_url', ['destination.ip', 'cs.uri.port', 'Label'], ['Label_HYBRID_1000_5'], aggfunc="count")
